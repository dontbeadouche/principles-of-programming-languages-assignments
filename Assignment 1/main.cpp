...
void listdata_not_ta(fstream& fout){
    ...
    buffer << setw(48) << "STUDENTS (WHO ARE NOT INSTRUCTORS)";
    buffer << setw(32) << "\n-------------------------------------------------------------";
    buffer << setw(12) << "\n";
    buffer << setw(6) << std::left << "ID";
    buffer << setw(20) << std::left << "NAME";
...
void listdata_ta(fstream& fout){
    ...
    buffer << setw(38) << "TEACHING ASSISTANTS";
    buffer << setw(32) << "\n-------------------------------------------------------------";
    buffer << setw(12) << "\n";
    buffer << setw(6) << std::left << "ID";
    buffer << setw(20) << std::left << "NAME";
    buffer << setw(60) << std::left << "COURSES_TAUGHT";
    buffer << setw(32) << "\n-------------------------------------------------------------\n";
    buffer << endl;
...
void read_employee_data(string filename){
    ...
    while(!fin.eof()){
        ...
        switch(id[0]){
            case 'E':   Instructor(id, name, salary); break;
            case 'S':   TeachingAssistant(id, name, 0, salary); break;
            default: cout << "Error";
        }
    }
    fin.close();
...
void read_instructor_data(string filename){
    ...
    while(!fin.eof()){
        ...
        switch(id[0]){
            case 'E':   Instructor::assignCourse(id, course); break;
            case 'S':   TeachingAssistant::assignCourse(id, course); break;
            default: cout << "Error";
        }
    }
    fin.close();
